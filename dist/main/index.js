...
const ioUtil = __nccwpck_require__(569);
...
const core = __importStar(__nccwpck_require__(493));
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const loglevel_1 = __importDefault(__nccwpck_require__(619));
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core_1 = __nccwpck_require__(493);
const io_1 = __nccwpck_require__(982);
const io_util_1 = __nccwpck_require__(569);
const path_1 = __importDefault(__nccwpck_require__(17));
const util_1 = __importDefault(__nccwpck_require__(837));
const getVars_1 = __nccwpck_require__(895);
const isErrorLike_1 = __nccwpck_require__(427);
const log_1 = __importDefault(__nccwpck_require__(661));
const exec = util_1.default.promisify((__nccwpck_require__(81).exec));
(async () => {
    try {
        ...
        await Promise.all(paths.map(async ({ path, cache, target }) => {
            if (await (0, io_util_1.exists)(cache)) {
                ...
                if (process.platform === 'linux' || process.platform === 'darwin') {
                    const isDir = await (0, io_util_1.isDirectory)(cache);
                    const cacheWithTrailingSlash = isDir && !cache.endsWith('/') ? cache + '/' : cache;
                    // This is faster, only for linux/mac
                    await exec(`cp ${isDir ? '-a' : ''} ${cacheWithTrailingSlash}${isDir ? '.' : ''} ${target}`);
                }
                else {
